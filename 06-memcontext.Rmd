...
```{r fig6.0, echo = FALSE, out.width='100%', fig.align = 'center', fig.alt = 'A double-exposure photograph of a young girl on a beach, with multiple transparent overlapping images of her in different expressive poses layered over the original.'}
...
```{r trafficsign, echo = FALSE, out.width='80%', fig.align = 'center', fig.cap = 'Misinformation can be introduced into the memory of a witness between the time of seeing an event and reporting it later. Something as straightforward as which sort of traffic sign was in place at an intersection can be confused if subjects are exposed to erroneous information after the initial incident. *Original photo by Kevin Lee on Unsplash; edited by C. Pilegard.*', fig.alt = 'Two identical photographs of a yellow taxi stopped at an intersection, side by side. In the left image the taxi is stopped at a stop sign; in the right image the stop sign has been replaced with a yield sign.'}
...
```{r lineup, echo = FALSE, out.width='60%', fig.align = 'center', fig.cap = 'Most lineups in the United States are conducted with photo spreads.', fig.alt = 'A grid of six identical generic silhouette portrait icons arranged in two rows of three, representing a police lineup identification task.'}
...
```{r nickel, echo = FALSE, out.width='80%', fig.align = 'center', fig.cap = 'Can you tell which coin, (a), (b), (c), or (d) is the accurate depiction of a US nickel? The correct answer is (c).', fig.alt = 'Four versions of the face side of a U.S. nickel, labeled A through D, with small differences in the orientation and placement of text and the portrait.'}
...
```{r interference, echo = FALSE, out.width='90%', fig.align = 'center', fig.cap = 'Examples of proactive and retroactive interference.', fig.alt = 'Diagram illustrating proactive and retroactive interference. The top row flows left to right: learning an old locker combination leads via proactive interference to difficulty recalling a new locker combination. The bottom row flows right to left: learning a sibling\'s new college email address leads via retroactive interference to difficulty recalling the old email address.'}
