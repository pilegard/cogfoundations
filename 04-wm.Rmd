...
```{r fig4.0, echo = FALSE, out.width='100%', fig.align = 'center', fig.alt = 'Photograph of an old wooden desk covered with papers, stamps, and glasses, suggesting a workspace.'}
...
```{r modalmodel, echo = FALSE, out.width='60%', fig.align = 'center', fig.cap = 'The simplest version of Atkinson and Shiffrin’s Modal Model of Memory.', fig.alt = 'Flowchart of Atkinson and Shiffrin\'s modal model of memory. Sensory memory feeds into short-term memory via attention, and short-term memory connects to long-term memory via encoding. Retrieval brings information back from long-term memory to short-term memory. A rehearsal loop cycles within short-term memory.'}
...
```{r chess, echo = FALSE, out.width='80%', fig.align = 'center', fig.cap = 'Left: A chessboard in the middle of a real game. Right: A random arrangement of chess pieces. Chess experts have significantly better memory for real boards, but perform more like novices for random board configurations.', fig.alt = 'Two chessboards side by side. The left shows a realistic mid-game arrangement of pieces as it might occur in actual play. The right shows the same pieces placed randomly, without any pattern reflecting real gameplay.'}
...
```{r baddeley, echo = FALSE, out.width='60%', fig.align = 'center', fig.cap = 'The original multicomponent working memory model from @BADDELEY197447.', fig.alt = 'Diagram of Baddeley\'s original multicomponent model of working memory. A central executive in the center connects via bidirectional arrows to two systems: the visuospatial sketchpad on the left and the phonological loop on the right.'}
...
```{r blockf, echo = FALSE, out.width='10%', fig.align = 'center', fig.cap = 'The block letter F task.', fig.alt = 'A block letter F with a star marking the bottom-left corner and an arrow pointing upward.'}
...
```{r episodic, echo = FALSE, out.width='50%', fig.align = 'center', fig.cap = 'An updated version of the multicomponent working memory model [@Baddeley2000].', fig.alt = 'Diagram of Baddeley\'s updated multicomponent model of working memory including the episodic buffer. The central executive sits at the top, connected by bidirectional arrows to three subsystems: the visuospatial sketchpad, the episodic buffer, and the phonological loop. Below, a long-term memory band connects via bidirectional arrows to visual semantics, episodic LTM, and language, with arrows also running between those three components.'}
