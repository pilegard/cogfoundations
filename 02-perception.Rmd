...
```{r fig2.0, echo = FALSE, out.width='100%', fig.align = 'center', fig.alt = 'A hand holds a glass sphere in front of a sunset. The sphere acts as a lens, showing a distorted, upside-down reflection of the sunset within it.'}
...
```{r anatomy, echo = FALSE, out.width='60%', fig.align = 'center', fig.cap = 'Anatomy of the Human Eye. Behind the pupil is the lens, a structure that focuses the incoming light on the retina, the layer of tissue at the back of the eye that contains photoreceptor cells. Rays from the top of the image strike the bottom of the retina and vice versa, and rays from the left side of the image strike the right part of the retina and vice versa, causing the image on the retina to be upside down.', fig.alt = 'Anatomical diagram of the human eye in cross-section, showing how light from a leaf passes through the cornea, iris, pupil, and lens before projecting an inverted image onto the retina. Labels indicate the iris, cornea, lens, retina, muscles, and optic nerve.'}
knitr::include_graphics("images/ch2/fig1.png")
```
...
```{r monalisa, echo = FALSE, out.width='30%', fig.align = 'center', fig.cap = "Mona Lisa's smile.", fig.alt = 'Close-up of the Mona Lisa\'s face, showing her famously ambiguous expression, dark eyes, and subtle smile against a softly lit complexion.'}
...
```{r blindspot, echo = FALSE, out.width='60%', fig.align = 'center', fig.cap = 'Blind Spot Demonstration. You can get an idea of the extent of your blind spot (the place where the optic nerve leaves the retina) by trying this: close your left eye and stare with your right eye at the cross in the diagram. You should be able to see the elephant image to the right (don’t look at it, just notice that it is there). If you can’t see the elephant, move closer or farther away until you can. Now slowly move so that you are closer to the image while you keep looking at the cross. At one distance (around a foot or so depending on your zoom), the elephant will completely disappear from view because its image has fallen on the blind spot.', fig.alt = 'A black plus sign on the left and a small illustration of an elephant on the right, used to demonstrate the visual blind spot.'}
...
```{r mcgurk, echo = FALSE, out.width='35%', fig.align = 'center', fig.cap = 'Watch [The McGurk Effect on YouTube](http://www.youtube.com/watch?v=jtsfidRq2tw)', fig.alt = 'A man faces the camera with his mouth open mid-speech.'}
...
```{r muellerlyer, echo = FALSE, out.width='60%', fig.align = 'center', fig.cap = 'The Mueller-Lyer illusion makes the line segment at the top of the left picture appear shorter than the one at the bottom. The illusion is caused, in part, by the monocular distance cue of depth — the bottom line looks like an edge that is normally farther away from us, whereas the top one looks like an edge that is normally closer.', fig.alt = 'Left: two horizontal lines of equal length illustrating the Müller-Lyer illusion -- the top line has outward-pointing arrowheads and appears shorter, while the bottom has inward-pointing arrowheads and appears longer. Right: a three-dimensional room sketch showing how the same lines resemble near and far edges in a real environment, suggesting a depth-based explanation for the illusion.'}
...
```{r ponzoillusion, echo = FALSE, out.width='60%', fig.align = 'center', fig.cap = 'The Ponzo Illusion. The Ponzo illusion is caused by a failure of the monocular depth cue of linear perspective. Both bars are the same size, even though the top one looks larger.', fig.alt = 'A sketch of two converging lines resembling receding train tracks, crossed by horizontal lines. Two yellow horizontal bars of equal length are overlaid at different depths; the higher bar appears longer due to the linear perspective cues surrounding it.'}
...
```{r humanfactors, echo = FALSE, out.width='80%', fig.align = 'center', fig.cap = 'Airplane instrument panel before (left) and after (right) it was redesigned by human factors psychologists.', fig.alt = 'Before and after images of an airplane cockpit redesign. The before image shows a dense arrangement of dials and gauges with high visual complexity. The after image shows a redesigned cockpit with the same essential controls organized more cleanly, reducing visual clutter.'}
